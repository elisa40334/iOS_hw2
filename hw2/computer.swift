...
struct computer: View {
    ...
    @Binding var compute:Bool
    @Binding var style:Bool
//    @State var arr = [0,0,0,0,1,1,6,1,1,0,0,0,0,4]
    ...
    @State var cal2 = 0
    @State var cal3 = 0
    @State var check0:[Int] = []
    @State var check0cal = 0
    @State var posi = true
    @State var re = false
    ...
    func f2(){
        ...
        if cal == 0 || cal2 == 0{
            ...
            num = Int(check0.randomElement()!)
            print(check0,num,arr,sum)
            cal3 = 0
            for kk in (0..<arr[num]){
                let aa = (kk+num+1)%14
                if aa != 6{
                    ...
                    cal3 += 1
                    var bb = (arr[num]+num+cal3)%14
                    arr[bb] += 1
                }
            }
            
            if arr[(arr[num]+num)%14] == 1 && (arr[num]+num)%14 >= 7 && (arr[num]+num)%14 <= 12 && arr[12-(arr[num]+num)%14] != 0{
                arr[13] += (arr[12-(arr[num]+num)%14]+1)
                arr[12-(arr[num]+num)%14] = 0
                arr[(arr[num]+num)%14] = 0
            }
            arr[num] = 0
...
//                                            Color(red: 0.98, green: 0.838, blue: 0.647)
                                            ZStack{
                                                Circle()
                                                    ...
                                                    .fullScreenCover(isPresented: $endd, content: {winlose(show: $compute,posi:$posi,win:$win,tie:$tie,p1: $arr[6],p2: $arr[13],style:$style)})
                                                    .onTapGesture {
...
struct computer_Previews: PreviewProvider {
    static var previews: some View {
        computer(compute: .constant(true),style: .constant(true))
    }
}
